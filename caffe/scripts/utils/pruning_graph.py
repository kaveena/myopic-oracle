...
class PruningConvolutionLayer(PruningLayer):
  ...
  def UpdateMaskInputChannel(self, idx_channel, final=True):
    self.active_ifm[idx_channel] = 0
    if final: 
...
class PruningInnerProductLayer(PruningLayer):
  ...
  def UpdateMaskInputChannel(self, idx_channel, final=True):
    ...
    self.active_ifm[idx_channel] = 0
    if final:
      self.caffe_layer.blobs[0].data[:, int(idx_channel * self.input_size) : int((idx_channel * self.input_size) + self.input_size)].fill(0)
    print("pruned input channel ", idx_channel, self.name)
